...
# -*- coding: utf-8 -*-
"""
Region of Interest (ROI) Statistics Extractor.

This script extracts statistical metrics (mean, std, median, quantiles, etc.)
from template images based on Allen ROI labels. It generates:
1. TSV/CSV tables summarizing statistics for each ROI per Group/Modality.
2. Optional PNG plots showing voxel value distributions per ROI.
"""
...
def _quantiles(vals: np.ndarray, qs: List[float]) -> np.ndarray:
    ...
    qs : List[float]
        List of quantiles to compute (e.g., [0.25, 0.75]).

    Returns
    -------
    np.ndarray
        Array of quantiles in the same order as qs.
    """
...
def load_label_table(csv_path: Path) -> Dict[int, str]:
    """
    Load a CSV table mapping ROI IDs to names.

    Parameters
    ----------
    csv_path : Path
        Path to the CSV file. Must contain 'id' and 'name' columns.

    Returns
    -------
    Dict[int, str]
        Dictionary mapping integer ROI IDs to their string names.
    """
...
def label_name_from_id(
    ...
    """
    Determine the base ID, hemisphere, and full name for a given ROI ID.

    Parameters
    ----------
    roi_id : int
        The raw ROI ID from the label image.
    id_to_name : Dict[int, str]
        Mapping from ID to base name.
    lr_offset : int
        Offset value distinguishing Left from Right (e.g., Left = ID + offset).
    right_suffix : str
        Suffix to append for Right hemisphere names.
    left_suffix : str
        Suffix to append for Left hemisphere names.

    Returns
    -------
    Tuple[int, str, str]
        (Base ID, Hemisphere 'L'/'R', Full Name)
    """
    if roi_id == 0:
        return 0, "", "Background"
...
def discover_templates(out_root: Path, modalities: List[str]) -> List[Tuple[str, str, Path]]:
    """
    Find template images in the output directory structure.
    Pattern: OUT_ROOT/derivatives/Brain_extracted/<MOD>/To_Template/<GROUP>/template/*_template.nii.gz

    Parameters
    ----------
    out_root : Path
        Root directory of the project outputs.
    modalities : List[str]
        List of modalities to search for (e.g., ['T1map', 'UNIT1']).

    Returns
    -------
    List[Tuple[str, str, Path]]
        List of tuples containing (group_name, modality_name, template_path).
    """
...
def compute_stats_fast(
    ...
    """
    Compute statistical metrics for each label in the label_data volume.
    ...
    value_data : np.ndarray
        3D array of scalar values (template data).
    include_negative : bool
        If False, negative values in value_data are excluded from stats.
    compute_minmax : bool
        If True, compute min and max values for each ROI.

    Returns
    -------
    Dict[int, Dict[str, float]]
        Dictionary where keys are ROI IDs and values are dictionaries of stats
        (mean, std, median, etc.).
    """
...
def parse_roi_ids(arg: str) -> Optional[List[int]]:
    """
    Parse a comma-separated string of ROI IDs.

    Parameters
    ----------
    arg : str
        Input string (e.g., "214,2214" or "all" or "").

    Returns
    -------
    Optional[List[int]]
        List of integer IDs, empty list for "all", or None if arg is empty.
    """
...
def write_group_modality_table(df: pd.DataFrame, outdir: Path, group: str, modality: str, as_csv: bool) -> Path:
    """
    Save the statistics DataFrame to a CSV or TSV file.
...
def plot_single_roi_distribution(
    ...
    """
    Generate and save a boxplot + strip plot for a single ROI.
    
    Parameters
    ----------
    values : np.ndarray
        Array of voxel values in the ROI.
    roi_id : int
        ID of the ROI.
    roi_name : str
        Name of the ROI.
    hemi : str
        Hemisphere ('L' or 'R').
    stats : Dict[str, float]
        Precomputed statistics for annotation.
    out_png : Path
        Output path for the PNG image.
    modality_label : str
        Label for the Y-axis.
    max_points : int
        Maximum number of points to plot (downsampling) for the jitter plot.
    """
    import matplotlib
    matplotlib.use("Agg")
...
def main() -> None:
    """
    Main entry point for extracting ROI statistics.
    """
    args = parse_args()
    ...
    if not template_list:
        ...
        logging.info(f"No templates found in To_Template for modalities: {modalities}. (Skipping stats if none found).")
        return
    ...
    for group, modality, tpl_path in template_list:
        ...
        logging.info(f"[OK] ROI table: {out_table}")
        ...
        if args.per_roi_png:
            ...
            logging.info(f"[OK] Per-ROI PNGs in: {per_roi_png_dir / group / modality}")
