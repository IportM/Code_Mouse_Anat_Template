...
def plot_single_roi_distribution(
    ...
    p05 = float(stats.get("p05", np.nan))
    p95 = float(stats.get("p95", np.nan))
    q1 = float(stats.get("q1", np.nan))
    median = float(stats.get("median", np.nan))
    q3 = float(stats.get("q3", np.nan))
    iqr = float(stats.get("iqr", np.nan))
    pct1 = float(stats.get("pct_within_1sd", np.nan))
    pctw = float(stats.get("pct_within_whiskers", np.nan))
    ...
    # Make box slightly transparent (closer to your reference)
    for box in bp["boxes"]:
        box.set_alpha(0.35)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--labels", type=str, default="", help="Default: ./Ressources/100_AMBA_LR.nii.gz next to this script.")
    p.add_argument("--labels-table", type=str, default="", help="Default: ./Ressources/allen_labels_table.csv next to this script.")
...
def main() -> None:
    ...
    labels_path = Path(args.labels).resolve() if args.labels else (script_dir / "Ressources/100_AMBA_LR.nii.gz")
    table_path = Path(args.labels_table).resolve() if args.labels_table else (script_dir / "Ressources/allen_labels_table.csv")
    ...
    if not template_list:
        raise RuntimeError("No template images found (check your template outputs).")
